 ...
 * @package bikes2ride
 */
if ( ! class_exists( 'bikes2ride_Carousel_Widget' ) ) {

	/**
	 * Class bikes2ride_Carousel_Widget.
	 */
	class bikes2ride_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of your posts on your site.', 'bikes2ride' );
			$this->widget_id          = 'bikes2ride_widget_carousel';
			$this->widget_name        = esc_html__( 'Carousel', 'bikes2ride' );
			...
			$this->settings           = array(
				...
				'slides_per_view' => array(
					...
					'min_value' => 1,
					'value'     => 5,
					'label'     => esc_html__( 'Number of slides per view', 'bikes2ride' ),
				),
				'slides_per_group' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Number slides per group', 'bikes2ride' ),
				),
				'slides_per_column' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Multi Row Slides Layout', 'bikes2ride' ),
				),
				'space_between_slides' => array(
					...
					'min_value' => 0,
					'value'     => 30,
					'label'     => esc_html__( 'Width of the space between slides(px)', 'bikes2ride' ),
				),
				'duration_speed' => array(
					...
					'min_value' => 100,
					'value'     => 500,
					'label'     => esc_html__( 'Duration of transition between slides (ms)', 'bikes2ride' ),
				),
				'navigation' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider navigation', 'bikes2ride' ),
				),
				'pagination' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider pagination', 'bikes2ride' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'bikes2ride_carousel_widget_view_dir', 'inc/widgets/carousel/views/carousel-view.php' ), false, false );
			...
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', bikes2ride_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="bikes2ride-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'bikes2ride' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'bikes2ride_carousel_default_query_args', array(
				'post_type'      => 'post',
				'orderby'        => 'date',
				'order'          => 'DESC',
				'posts_per_page' => - 1,
		...
		public function get_carousel_loop( $posts_query, $template ) {
			...
			if ( ! $template  ) {
				return '<h5>' . esc_html__( 'View file not found', 'bikes2ride' ) . '</h5>';
			}
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				...
				$image = $this->utility->media->get_image( array(
					'size'              => 'post-thumbnail',
					'mobile_size'       => 'bikes2ride-thumb-m',
	...
	add_action( 'widgets_init', 'bikes2ride_register_carousel_widget' );
	/**
	 * Register carousel widget.
	 */
	function bikes2ride_register_carousel_widget() {
		register_widget( 'bikes2ride_Carousel_Widget' );
	}
}
