...
if ( ! class_exists( 'bikes2ride_Custom_Posts_Widget' ) ) {

	/**
	 * Class bikes2ride_Custom_Posts_Widget.
	 */
	class bikes2ride_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'bikes2ride' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'bikes2ride' );
			$this->widget_id          = apply_filters( 'bikes2ride_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'bikes2ride_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = bikes2ride_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'bikes2ride' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'bikes2ride' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'bikes2ride' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'bikes2ride' ),
				...
				'post_offset' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'bikes2ride' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'bikes2ride' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						...
						'comment_count' => esc_html__( 'Comment count', 'bikes2ride' ),
						'category'      => esc_html__( 'Category', 'bikes2ride' ),
						'post_tag'      => esc_html__( 'Tag', 'bikes2ride' ),
						'more_button'   => esc_html__( 'More Button', 'bikes2ride' ),
					),
				...
				'button_text' => array(
					...
					'label' => esc_html__( 'Post read more button label', 'bikes2ride' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'bikes2ride_custom_posts_widget_view_dir', 'inc/widgets/custom-posts/views/custom-post-view.php' ), false, false );
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', bikes2ride_theme()->customizer->get_default( 'footer_widget_columns' ) );
	...
	add_action( 'widgets_init', 'bikes2ride_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function bikes2ride_register_custom_posts_widget() {
		register_widget( 'bikes2ride_Custom_Posts_Widget' );
	}
}
