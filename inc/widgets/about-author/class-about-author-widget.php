 ...
 * @package bikes2ride
 */
if ( ! class_exists( 'bikes2ride_About_Author_Widget' ) ) {

	/**
	 * Class bikes2ride_About_Author_Widget.
	 */
	class bikes2ride_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'bikes2ride widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'bikes2ride' );
			$this->widget_id          = 'bikes2ride_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'bikes2ride' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'bikes2ride' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'bikes2ride' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'bikes2ride' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'bikes2ride' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'bikes2ride' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'bikes2ride' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'bikes2ride' ),
					'label' => esc_html__( 'Link label', 'bikes2ride' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'bikes2ride' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'bikes2ride-author-avatar' );
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'bikes2ride_widget_about_author_view', 'inc/widgets/about-author/view/about-author.php' ), false, false );
...
add_action( 'widgets_init', 'bikes2ride_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function bikes2ride_register_about_author_widgets() {
	register_widget( 'bikes2ride_About_Author_Widget' );
}
