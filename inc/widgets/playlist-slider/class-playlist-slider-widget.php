...
if ( ! class_exists( 'bikes2ride_Playlist_Slider_Widget' ) ) {

	/**
	 * Class bikes2ride_Playlist_Slider_Widget.
	 */
	class bikes2ride_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'bikes2ride' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'bikes2ride' );
			$this->widget_id          = apply_filters( 'bikes2ride_playlist_slider_widget_ID', 'bikes2ride_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'bikes2ride_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = bikes2ride_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'bikes2ride' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'bikes2ride' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'bikes2ride' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '500',
					'label' => esc_html__( 'Slider height ( px, rem )', 'bikes2ride' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'bikes2ride' ),
						true  => esc_html__( 'Buttons', 'bikes2ride' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'bikes2ride' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'bikes2ride' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'bikes2ride' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'bikes2ride' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'bikes2ride' ),
				),
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'bikes2ride' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'     => esc_html__( 'Date', 'bikes2ride' ),
						'author'   => esc_html__( 'Author', 'bikes2ride' ),
						'comments' => esc_html__( 'Comment count', 'bikes2ride' ),
						'category' => esc_html__( 'Category', 'bikes2ride' ),
						'post_tag' => esc_html__( 'Tag', 'bikes2ride' ),
					),
					'label' => esc_html__( 'Display post meta data', 'bikes2ride' ),
				),
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'bikes2ride' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'bikes2ride_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			...
			foreach ( $this->posts as $post ) {
				...
				$image = $this->utility->media->get_image( array(
					'size'        => 'bikes2ride-thumb-s',
					'mobile_size' => 'bikes2ride-thumb-s',
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'bikes2ride_playlist_buttons', false ),
...
add_action( 'widgets_init', 'bikes2ride_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function bikes2ride_register_playlist_slider_widget() {
	register_widget( 'bikes2ride_Playlist_Slider_Widget' );
}
