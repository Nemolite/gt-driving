...
class bikes2ride_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'bikes2ride' );
		$this->widget_id          = 'bikes2ride_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'bikes2ride' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'bikes2ride' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'bikes2ride' ),
				'label' => esc_html__( 'Subscribe Title', 'bikes2ride' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'bikes2ride' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'bikes2ride' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'bikes2ride' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'bikes2ride' ),
				'label' => esc_html__( 'Subscribe submit label', 'bikes2ride' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'bikes2ride' ),
				'label' => esc_html__( 'Subscribe success', 'bikes2ride' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'bikes2ride' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'bikes2ride' ),
				'label' => esc_html__( 'Follow Title', 'bikes2ride' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'bikes2ride' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'bikes2ride' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'bikes2ride' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'bikes2ride' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'bikes2ride' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'bikes2ride' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'bikes2ride' ),
					'top-center'    => esc_html__( 'Top Center', 'bikes2ride' ),
					'top-right'     => esc_html__( 'Top Right', 'bikes2ride' ),
					'center-left'   => esc_html__( 'Middle Left', 'bikes2ride' ),
					'center'        => esc_html__( 'Middle Center', 'bikes2ride' ),
					'center-right'  => esc_html__( 'Middle Right', 'bikes2ride' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'bikes2ride' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'bikes2ride' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'bikes2ride' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'bikes2ride' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'bikes2ride' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'bikes2ride' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'bikes2ride' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'bikes2ride' ),
					'contain' => esc_html__( 'Contain', 'bikes2ride' ),
					'auto'    => esc_html__( 'Auto', 'bikes2ride' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'bikes2ride' ),
				'value'  => '#000000',
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_bikes2ride_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_bikes2ride_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'bikes2ride' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'bikes2ride' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'bikes2ride' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'bikes2ride' ),
		);

		$this->nonce = wp_create_nonce( 'bikes2ride-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		$follow_template            = locate_template( apply_filters( 'bikes2ride_subscribe_widget_follow_template_file', 'inc/widgets/subscribe-follow/view/follow-view.php' ), false, false );
		$subscribe_template         = locate_template( apply_filters( 'bikes2ride_subscribe_widget_subscribe_template_file', 'inc/widgets/subscribe-follow/view/subcribe-view.php' ), false, false );
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'bikes2ride' );
		}
	...
	public function get_social_nav() {
		return bikes2ride_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = bikes2ride_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'bikes2ride-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'bikes2ride_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function bikes2ride_register_subscribe_follow_widgets() {
	register_widget( 'bikes2ride_Subscribe_Follow_Widget' );
}
