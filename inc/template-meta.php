 ...
 * @return array
 */
function bikes2ride_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! bikes2ride_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'bikes2ride_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'bikes2ride_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'bikes2ride' ) . $network['name'],
 ...
 * @return string
 */
function bikes2ride_get_single_share_buttons() {
	ob_start();
